...
cd ../frontend
vercel env add NEXT_PUBLIC_API_BASE_URL production --value "https://your-backend.vercel.app" --visibility config --no-sensitive --yes
vercel deploy --yes --prod  # NEXT_PUBLIC_* vars are inlined at build time, so this must run after the env var is set
```
